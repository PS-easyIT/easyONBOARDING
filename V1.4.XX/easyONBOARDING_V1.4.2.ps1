...
if (-not (Test-Path $xamlPath)) {
    Write-LogMessage "XAML file not found: $xamlPath" -logLevel ERROR
...
try {
    ...
    if (-not $window) {
        Throw "XAML could not be loaded or is empty."
    }
    Write-LogMessage "XAML file loaded successfully from $xamlPath." -logLevel INFO
}
catch {
    $errorMsg = "Error loading XAML file: $($_.Exception.Message). Please check the file content and path ($xamlPath)."
    Write-LogMessage $errorMsg -logLevel ERROR
    # Attempt to show a message box if PresentationFramework is loaded
    try { [System.Windows.MessageBox]::Show($errorMsg, "XAML Load Error", "OK", "Error") } catch {}
    Throw "Critical Error: $errorMsg. StackTrace: $($_.ScriptStackTrace)"
}
#endregion


#region [Region 07 | GUI CONTROL VARIABLES] // NEU EINGEFÜGT
# PowerShell variables mapped to XAML control elements for easy access
Write-DebugMessage "Assigning GUI control variables."
try {
    $WPFwindow = $window # Hauptfenster

    # TabControl und Tabs
    $WPFMainTabControl = $window.FindName('MainTabControl'); if ($null -eq $WPFMainTabControl) { Throw "Control 'MainTabControl' not found." }
    $WPFTab_UserData = $WPFMainTabControl.FindName('Tab_UserData'); if ($null -eq $WPFTab_UserData) { Throw "Control 'Tab_UserData' not found." }
    $WPFTab_SettingsGroups = $WPFMainTabControl.FindName('Tab_SettingsGroups'); if ($null -eq $WPFTab_SettingsGroups) { Throw "Control 'Tab_SettingsGroups' not found." }

    # --- Tab 1: Benutzerdaten ---
    $WPFcmbOU = $WPFTab_UserData.FindName('cmbOU'); if ($null -eq $WPFcmbOU) { Throw "Control 'cmbOU' not found." }
    $WPFbtnRefreshOU = $WPFTab_UserData.FindName('btnRefreshOU'); if ($null -eq $WPFbtnRefreshOU) { Throw "Control 'btnRefreshOU' not found." }
    $WPFcmbDepartment = $WPFTab_UserData.FindName('cmbDepartment'); if ($null -eq $WPFcmbDepartment) { Throw "Control 'cmbDepartment' not found." } # NEU
    $WPFtxtFirstName = $WPFTab_UserData.FindName('txtFirstName'); if ($null -eq $WPFtxtFirstName) { Throw "Control 'txtFirstName' not found." }
    $WPFtxtLastName = $WPFTab_UserData.FindName('txtLastName'); if ($null -eq $WPFtxtLastName) { Throw "Control 'txtLastName' not found." }
    $WPFchkExternal = $WPFTab_UserData.FindName('chkExternal'); if ($null -eq $WPFchkExternal) { Throw "Control 'chkExternal' not found." }
    $WPFtxtDisplayName = $WPFTab_UserData.FindName('txtDisplayName'); if ($null -eq $WPFtxtDisplayName) { Throw "Control 'txtDisplayName' not found." }
    $WPFcmbDisplayTemplate = $WPFTab_UserData.FindName('cmbDisplayTemplate'); if ($null -eq $WPFcmbDisplayTemplate) { Throw "Control 'cmbDisplayTemplate' not found." }
    $WPFtxtDescriptionNumber = $WPFTab_UserData.FindName('txtDescriptionNumber'); if ($null -eq $WPFtxtDescriptionNumber) { Throw "Control 'txtDescriptionNumber' not found." }
    $WPFtxtDescription = $WPFTab_UserData.FindName('txtDescription'); if ($null -eq $WPFtxtDescription) { Throw "Control 'txtDescription' not found." }
    $WPFtxtOffice = $WPFTab_UserData.FindName('txtOffice'); if ($null -eq $WPFtxtOffice) { Throw "Control 'txtOffice' not found." }
    $WPFtxtPhone = $WPFTab_UserData.FindName('txtPhone'); if ($null -eq $WPFtxtPhone) { Throw "Control 'txtPhone' not found." }
    $WPFtxtMobile = $WPFTab_UserData.FindName('txtMobile'); if ($null -eq $WPFtxtMobile) { Throw "Control 'txtMobile' not found." }
    $WPFtxtPosition = $WPFTab_UserData.FindName('txtPosition'); if ($null -eq $WPFtxtPosition) { Throw "Control 'txtPosition' not found." }
    $WPFtxtDepartmentAD = $WPFTab_UserData.FindName('txtDepartmentAD'); if ($null -eq $WPFtxtDepartmentAD) { Throw "Control 'txtDepartmentAD' not found." } # Angepasst
    $WPFdatTermination = $WPFTab_UserData.FindName('datTermination'); if ($null -eq $WPFdatTermination) { Throw "Control 'datTermination' not found." }
    $WPFtxtEmailPrefix = $WPFTab_UserData.FindName('txtEmailPrefix'); if ($null -eq $WPFtxtEmailPrefix) { Throw "Control 'txtEmailPrefix' not found." } # NEU
    $WPFcmbSuffix = $WPFTab_UserData.FindName('cmbSuffix'); if ($null -eq $WPFcmbSuffix) { Throw "Control 'cmbSuffix' not found." }
    $WPFtxtEmail = $WPFTab_UserData.FindName('txtEmail'); if ($null -eq $WPFtxtEmail) { Throw "Control 'txtEmail' not found." } # Jetzt ReadOnly
    $WPFbtnImportCSV = $WPFTab_UserData.FindName('btnImportCSV'); if ($null -eq $WPFbtnImportCSV) { Throw "Control 'btnImportCSV' not found." }
    $WPFbtnClearForm = $WPFTab_UserData.FindName('btnClearForm'); if ($null -eq $WPFbtnClearForm) { Throw "Control 'btnClearForm' not found." } # Angepasst

    # --- Tab 2: Einstellungen & Gruppen ---
    $WPFrbAccountEnabled = $WPFTab_SettingsGroups.FindName('rbAccountEnabled'); if ($null -eq $WPFrbAccountEnabled) { Throw "Control 'rbAccountEnabled' not found." }
    $WPFrbAccountDisabled = $WPFTab_SettingsGroups.FindName('rbAccountDisabled'); if ($null -eq $WPFrbAccountDisabled) { Throw "Control 'rbAccountDisabled' not found." }
    $WPFchkTL = $WPFTab_SettingsGroups.FindName('chkTL'); if ($null -eq $WPFchkTL) { Throw "Control 'chkTL' not found." }
    $WPFchkAL = $WPFTab_SettingsGroups.FindName('chkAL'); if ($null -eq $WPFchkAL) { Throw "Control 'chkAL' not found." }
    $WPFcmbTLGroup = $WPFTab_SettingsGroups.FindName('cmbTLGroup'); if ($null -eq $WPFcmbTLGroup) { Throw "Control 'cmbTLGroup' not found." }
    $WPFrbRand = $WPFTab_SettingsGroups.FindName('rbRand'); if ($null -eq $WPFrbRand) { Throw "Control 'rbRand' not found." }
    $WPFtxtPWLen = $WPFTab_SettingsGroups.FindName('txtPWLen'); if ($null -eq $WPFtxtPWLen) { Throw "Control 'txtPWLen' not found." }
    $WPFchkIncludeSpecial = $WPFTab_SettingsGroups.FindName('chkIncludeSpecial'); if ($null -eq $WPFchkIncludeSpecial) { Throw "Control 'chkIncludeSpecial' not found." }
    $WPFchkAvoidAmbig = $WPFTab_SettingsGroups.FindName('chkAvoidAmbig'); if ($null -eq $WPFchkAvoidAmbig) { Throw "Control 'chkAvoidAmbig' not found." }
    $WPFrbFix = $WPFTab_SettingsGroups.FindName('rbFix'); if ($null -eq $WPFrbFix) { Throw "Control 'rbFix' not found." }
    $WPFtxtFixPW = $WPFTab_SettingsGroups.FindName('txtFixPW'); if ($null -eq $WPFtxtFixPW) { Throw "Control 'txtFixPW' not found." }
    $WPFcmbLicense = $WPFTab_SettingsGroups.FindName('cmbLicense'); if ($null -eq $WPFcmbLicense) { Throw "Control 'cmbLicense' not found." }
    $WPFicADGroups = $WPFTab_SettingsGroups.FindName('icADGroups'); if ($null -eq $WPFicADGroups) { Throw "Control 'icADGroups' not found." } # ItemsControl
    $WPFpicLogo1 = $WPFTab_SettingsGroups.FindName('picLogo1'); if ($null -eq $WPFpicLogo1) { Write-LogMessage "Optional control 'picLogo1' not found." -logLevel DEBUG } # Logo Tab 2 (Optional?)
    $WPFbtnUploadLogo = $WPFTab_SettingsGroups.FindName('btnUploadLogo'); if ($null -eq $WPFbtnUploadLogo) { Throw "Control 'btnUploadLogo' not found." }
    $WPFbtnPDF = $WPFTab_SettingsGroups.FindName('btnPDF'); if ($null -eq $WPFbtnPDF) { Throw "Control 'btnPDF' not found." }
    $WPFprogressBar = $WPFTab_SettingsGroups.FindName('progressBar'); if ($null -eq $WPFprogressBar) { Throw "Control 'progressBar' not found." }
    $WPFlblStatus = $WPFTab_SettingsGroups.FindName('lblStatus'); if ($null -eq $WPFlblStatus) { Throw "Control 'lblStatus' not found." }

    # --- Untere Button-Leiste ---
    $WPFbtnPreview = $window.FindName('btnPreview'); if ($null -eq $WPFbtnPreview) { Throw "Control 'btnPreview' not found." } # NEU
    $WPFbtnCreateUser = $window.FindName('btnCreateUser'); if ($null -eq $WPFbtnCreateUser) { Throw "Control 'btnCreateUser' not found." } # Angepasst
    $WPFbtnClose = $window.FindName('btnClose'); if ($null -eq $WPFbtnClose) { Throw "Control 'btnClose' not found." }
    
    Write-LogMessage "GUI control variables assigned successfully." -logLevel INFO
} catch {
    $errorMsg = "Error assigning GUI control variables: $($_.Exception.Message). Check if XAML controls match variable names."
    Write-LogMessage $errorMsg -logLevel ERROR
    try { [System.Windows.MessageBox]::Show($errorMsg, "Variable Assignment Error", "OK", "Error") } catch {}
    Throw "Critical Error: $errorMsg. StackTrace: $($_.ScriptStackTrace)"
}
#endregion

#region [Region 08 | INITIAL GUI POPULATION]
# Populates GUI elements with data from INI or defaults
Write-DebugMessage "Starting initial GUI population."
try {
    # Populate OU dropdown
    Load-OUs -cmbOU $WPFcmbOU -ScriptDir $ScriptDir
    
    # Populate Department dropdown (NEU)
    Load-Departments

    # Populate Display Name Template dropdown
    # Ensure the variable mapping reflects the new Tab structure if needed
    # Example: $comboBoxDisplayTemplate = $WPFTab_UserData.FindName("cmbDisplayTemplate")
    Load-DisplayNameTemplates -comboBox $WPFcmbDisplayTemplate

    # Populate License dropdown
    Load-LicenseOptions -comboBox $WPFcmbLicense

    # Populate TL Group dropdown
    Load-TLGroups -comboBox $WPFcmbTLGroup

    # Populate AD Groups Checkboxes
    Load-ADGroups

    # Populate Mail Suffix Dropdown
    Load-MailSuffixes -comboBox $WPFcmbSuffix
    ...
    # Set default values from INI if available
    Set-DefaultValues

    Write-LogMessage "Initial GUI population completed successfully."
} catch {
    Write-LogMessage "Error during initial GUI population: $($_.Exception.Message)" -logLevel ERROR
    [System.Windows.MessageBox]::Show("Failed to initialize some GUI elements. Please check the logs. Error: $($_.Exception.Message)", "Initialization Error", "OK", "Error")
}
Write-DebugMessage "Initial GUI population finished."
#endregion

# [Region 09 | EVENT HANDLERS]
# Defines actions triggered by user interactions with GUI elements
Write-DebugMessage "Assigning event handlers."

# --- Tab 1 Event Handlers ---
$WPFbtnRefreshOU.Add_Click({ 
    Write-LogMessage "Refresh OU button clicked."
    Load-OUs -cmbOU $WPFcmbOU -ScriptDir $ScriptDir 
})

# NEU: Event Handler für Abteilungsauswahl
$WPFcmbDepartment.Add_SelectionChanged({ 
    param($sender, $e)
    Write-LogMessage "Department selection changed."
    Handle-DepartmentSelectionChange
})

$WPFtxtFirstName.Add_TextChanged({
    Update-DisplayNameAndEmail
})
...
$WPFcmbDisplayTemplate.Add_SelectionChanged({
    Update-DisplayNameAndEmail
})

$WPFcmbSuffix.Add_SelectionChanged({
    Update-DisplayNameAndEmail # Update Email if suffix changes
})

$WPFbtnImportCSV.Add_Click({ 
    Write-LogMessage "Import CSV button clicked."
    Import-CSVData 
})

$WPFbtnClearForm.Add_Click({ 
    Write-LogMessage "Clear Form button clicked."
    Clear-OnboardingForm 
})

# --- Tab 2 Event Handlers ---
$WPFrbRand.Add_Checked({ $WPFtxtFixPW.IsEnabled = $false; $WPFtxtFixPW.Text = "" })
$WPFrbFix.Add_Checked({ $WPFtxtFixPW.IsEnabled = $true })

$WPFchkTL.Add_Checked({ $WPFcmbTLGroup.IsEnabled = $true })
$WPFchkTL.Add_Unchecked({ $WPFcmbTLGroup.IsEnabled = $false; $WPFcmbTLGroup.SelectedIndex = -1 })

$WPFbtnUploadLogo.Add_Click({ 
    Write-LogMessage "Upload Logo button clicked."
    # Implement logo upload logic here
    [System.Windows.MessageBox]::Show("Logo upload functionality not yet implemented.", "Info")
})

$WPFbtnPDF.Add_Click({ 
    Write-LogMessage "Create PDF button clicked."
    Create-OnboardingPDF # Call function to generate PDF
})

# --- Bottom Bar Event Handlers ---
$WPFbtnPreview.Add_Click({ 
    Write-LogMessage "Preview button clicked."
    Preview-UserData # Function to show a preview/summary
})

$WPFbtnCreateUser.Add_Click({ 
    param($sender, $e)
    Write-LogMessage "Create User button clicked."
    Start-UserOnboardingProcess
})

$WPFbtnClose.Add_Click({ 
    Write-LogMessage "Close button clicked. Exiting application."
    $WPFwindow.Close() 
})

Write-DebugMessage "Event handlers assigned."
#endregion

#region [Region 010 | UPN GENERATION]
    ...
    switch ($upnTemplate) {
        ...
        "FIRSTNAMELASTINITIAL"  { 
                    if (-not [string]::IsNullOrWhiteSpace($lastName)) {
                        ...
                        $SamAccountName = "$firstName" + "x".ToLower() + $adDomain
                    }
                }
        default                 { 
...
#region [Region 11 | AD USER CREATION]
...
function New-ADUserAccount {
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($UserData.EmailAddress)) {
            ...
            # If setProxyMailMailAddress is enabled, add proxyAddresses
...
#region [Region 12 | AD COMMAND EXECUTION]
...
#region [Region 13 | CONFIGURATION VALUE ACCESS]
...
#region [Region 14 | AD GROUP LOADING]
