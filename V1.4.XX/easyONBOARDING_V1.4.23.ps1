...
$INIPath = Join-Path $ScriptDir "./assets/easyONB.ini"
try {
...
#region [Region 03.3 | AD CONNECTION HELPER]
# Helper function to test AD connectivity before operations
function Test-ADConnectivity {
    ...
    try {
        # Check if a specific AD server is configured
        $adServer = $null
        if ($global:Config -and $global:Config.Contains("ActiveDirectory") -and 
            $global:Config.ActiveDirectory.Contains("PreferredDomainController") -and
            -not [string]::IsNullOrWhiteSpace($global:Config.ActiveDirectory.PreferredDomainController)) {
            $adServer = $global:Config.ActiveDirectory.PreferredDomainController
        }
        
        # Quick test of AD connectivity
        if ($adServer) {
            $null = Get-ADDomain -Server $adServer -ErrorAction Stop
        } else {
        ...
        if ($ShowError) {
            Write-Log -Message "AD connectivity test failed: $($_.Exception.Message)" -LogLevel "ERROR"
        }
        return $false
    }
}
...
function Get-ADConnectionStatus {
    ...
    if ($global:ADConnectionAvailable -eq $null) {
        $global:ADConnectionAvailable = Test-ADConnectivity
    }
    
    return $global:ADConnectionAvailable
}
#endregion
...
try {
    ...
    # Test AD connectivity with better error handling
    try {
        Write-DebugMessage "Testing Active Directory connectivity..."
        ...
        if ($global:Config -and $global:Config.Contains("ActiveDirectory") -and 
            ...
            -not [string]::IsNullOrWhiteSpace($global:Config.ActiveDirectory.PreferredDomainController)) {
            $adServer = $global:Config.ActiveDirectory.PreferredDomainController
            Write-DebugMessage "Using configured AD server: $adServer"
        }
        
        # Test connection with or without specific server
        if ($adServer) {
            $adDomain = Get-ADDomain -Server $adServer -ErrorAction Stop
        } else {
            $adDomain = Get-ADDomain -ErrorAction Stop
        }
        
        Write-DebugMessage "Connected to AD Domain: $($adDomain.DNSRoot)"
        $global:ADConnectionAvailable = $true
    } catch {
        Write-Log -Message "AD connectivity test failed: $($_.Exception.Message)" -LogLevel "WARN"
        Write-DebugMessage "AD connectivity test failed, but module is loaded. Some features may be limited."
        $global:ADConnectionAvailable = $false
        
        # Try to suppress the warning about default server
        try {
            # Set a dummy AD drive to avoid automatic connection attempts
            $null = New-PSDrive -Name "AD_Dummy" -PSProvider ActiveDirectory -Root "" -ErrorAction SilentlyContinue
        } catch {
            Write-DebugMessage "Could not create AD dummy drive: $($_.Exception.Message)"
        }
    }
} catch {
    ...
    # Check if this is a critical error or if we can continue
    if ($_.Exception.Message -match "module.*not.*found|could not be loaded") {
        # Module installation issue
        Write-Log -Message "ActiveDirectory module is not installed or not available." -LogLevel "ERROR"
        $continueWithoutAD = $false
    } else {
        # Connection issue, module is available but AD is not reachable
        Write-Log -Message "ActiveDirectory module loaded but AD services not available." -LogLevel "WARN"
        $continueWithoutAD = $true
    }
    
    if (-not $continueWithoutAD) {
        Throw "Critical error: ActiveDirectory module missing or not installed!"
    } else {
        Write-DebugMessage "Continuing without full AD connectivity - some features may be limited."
    }
}
#endregion

#region [Region 04.1 | CUSTOM MODULES]
# Loads custom modules for enhanced functionality
Write-DebugMessage "Loading custom modules."
try {
    # Check if Modules directory exists
    $ModulesPath = Join-Path $ScriptDir "Modules"
    if (-not (Test-Path $ModulesPath)) {
        Write-Log -Message "Modules directory not found: $ModulesPath" -LogLevel "WARN"
        Write-DebugMessage "Modules directory missing, creating it."
        New-Item -ItemType Directory -Path $ModulesPath -Force | Out-Null
    }
    
    # Import SettingsManager module
    $SettingsManagerPath = Join-Path $ModulesPath "SettingsManager.psm1"
    if (Test-Path $SettingsManagerPath) {
        Import-Module $SettingsManagerPath -Force -ErrorAction Stop
        Write-DebugMessage "SettingsManager module loaded successfully."
    } else {
        Write-Log -Message "SettingsManager module not found: $SettingsManagerPath" -LogLevel "WARN"
    }
    
    # Import ADManager module
    $ADManagerPath = Join-Path $ModulesPath "ADManager.psm1"
    if (Test-Path $ADManagerPath) {
        Import-Module $ADManagerPath -Force -ErrorAction Stop
        Write-DebugMessage "ADManager module loaded successfully."
    } else {
        Write-Log -Message "ADManager module not found: $ADManagerPath" -LogLevel "WARN"
    }
    
    # Import UIManager module
    $UIManagerPath = Join-Path $ModulesPath "UIManager.psm1"
    if (Test-Path $UIManagerPath) {
        Import-Module $UIManagerPath -Force -ErrorAction Stop
        Write-DebugMessage "UIManager module loaded successfully."
    } else {
        Write-Log -Message "UIManager module not found: $UIManagerPath" -LogLevel "WARN"
    }
    
    # Import ExtendedFunctions module
    $ExtendedFunctionsPath = Join-Path $ModulesPath "ExtendedFunctions.psm1"
    if (Test-Path $ExtendedFunctionsPath) {
        Import-Module $ExtendedFunctionsPath -Force -ErrorAction Stop
        Write-DebugMessage "ExtendedFunctions module loaded successfully."
        
        # Initialize the module
        Initialize-ExtendedFunctions -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
    } else {
        Write-Log -Message "ExtendedFunctions module not found: $ExtendedFunctionsPath" -LogLevel "WARN"
    }
    
    # Import ExtendedFunctions2 module (additional functions)
    $ExtendedFunctions2Path = Join-Path $ModulesPath "ExtendedFunctions2.psm1"
    if (Test-Path $ExtendedFunctions2Path) {
        Import-Module $ExtendedFunctions2Path -Force -ErrorAction Stop
        Write-DebugMessage "ExtendedFunctions2 module loaded successfully."
    } else {
        Write-Log -Message "ExtendedFunctions2 module not found: $ExtendedFunctions2Path" -LogLevel "WARN"
    }
    
    # Import SettingsGUI module
    $SettingsGUIPath = Join-Path $ModulesPath "SettingsGUI.psm1"
    if (Test-Path $SettingsGUIPath) {
        Import-Module $SettingsGUIPath -Force -ErrorAction Stop
        Write-DebugMessage "SettingsGUI module loaded successfully."
        
        # Initialize the module
        Initialize-SettingsGUI -IniPath $INIPath -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
    } else {
        Write-Log -Message "SettingsGUI module not found: $SettingsGUIPath" -LogLevel "WARN"
    }
    
    # Import SettingsGUI2 module (additional panels)
    $SettingsGUI2Path = Join-Path $ModulesPath "SettingsGUI2.psm1"
    if (Test-Path $SettingsGUI2Path) {
        Import-Module $SettingsGUI2Path -Force -ErrorAction Stop
        Write-DebugMessage "SettingsGUI2 module loaded successfully."
    } else {
        Write-Log -Message "SettingsGUI2 module not found: $SettingsGUI2Path" -LogLevel "WARN"
    }
    
    # Import ReportingModule
    $ReportingModulePath = Join-Path $ModulesPath "ReportingModule.psm1"
    if (Test-Path $ReportingModulePath) {
        Import-Module $ReportingModulePath -Force -ErrorAction Stop
        Write-DebugMessage "ReportingModule loaded successfully."
        ...
        Initialize-ReportingModule -ReportPath $reportPath -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
    } else {
        Write-Log -Message "ReportingModule not found: $ReportingModulePath" -LogLevel "WARN"
    }
    
    # Import ValidationModule
    $ValidationModulePath = Join-Path $ModulesPath "ValidationModule.psm1"
    if (Test-Path $ValidationModulePath) {
        Import-Module $ValidationModulePath -Force -ErrorAction Stop
        Write-DebugMessage "ValidationModule loaded successfully."
        
        # Initialize the module with custom rules from INI if available
        $customRules = @{}
        if ($global:Config.Contains("ValidationRules")) {
            $customRules = $global:Config["ValidationRules"]
        }
        Initialize-ValidationModule -CustomRules $customRules -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
    } else {
        Write-Log -Message "ValidationModule not found: $ValidationModulePath" -LogLevel "WARN"
    }
    
    # Import NotificationModule
    $NotificationModulePath = Join-Path $ModulesPath "NotificationModule.psm1"
    if (Test-Path $NotificationModulePath) {
        Import-Module $NotificationModulePath -Force -ErrorAction Stop
        Write-DebugMessage "NotificationModule loaded successfully."
        
        # Initialize the module with email config from INI
        $emailConfig = @{}
        $teamsConfig = @{}
        if ($global:Config.Contains("EmailSettings")) {
            $emailConfig = $global:Config["EmailSettings"]
        }
        if ($global:Config.Contains("TeamsSettings")) {
            $teamsConfig = $global:Config["TeamsSettings"]
        }
        Initialize-NotificationModule -EmailConfiguration $emailConfig -TeamsConfiguration $teamsConfig -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
    } else {
        Write-Log -Message "NotificationModule not found: $NotificationModulePath" -LogLevel "WARN"
    }
    
    # Import LicenseModule
    $LicenseModulePath = Join-Path $ModulesPath "LicenseModule.psm1"
    if (Test-Path $LicenseModulePath) {
        Import-Module $LicenseModulePath -Force -ErrorAction Stop
        Write-DebugMessage "LicenseModule loaded successfully."
        
        # Initialize the module with license config from INI
        $licenseConfig = @{}
        if ($global:Config.Contains("LicenseSettings")) {
            $licenseConfig = $global:Config["LicenseSettings"]
        }
        Initialize-LicenseModule -Configuration $licenseConfig -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
    } else {
        Write-Log -Message "LicenseModule not found: $LicenseModulePath" -LogLevel "WARN"
    }
    
    Write-DebugMessage "Custom modules loading completed."
} catch {
    Write-Log -Message "Error loading custom modules: $($_.Exception.Message)" -LogLevel "ERROR"
    # Continue execution as these modules are optional enhancements
}
#endregion
...
function Load-ADGroups {
    try {
        ...
        # Find the onboarding panel first
        $onboardingPanel = $window.FindName("onboardingPanel")
        if ($null -eq $onboardingPanel) {
            Write-Error "Onboarding panel not found. Check the XAML structure."
            return
        }
        
        # Try to find dgGroups DataGrid within the onboarding panel
        $dgGroups = $window.FindName("dgGroups")
        
        # If not found, try the old icADGroups for backward compatibility
        if ($null -eq $dgGroups) {
            $icADGroups = $onboardingPanel.FindName("icADGroups")
            if ($null -eq $icADGroups) {
                Write-DebugMessage "Neither dgGroups nor icADGroups found in XAML structure"
                Write-Warning "AD Groups control not found. AD Groups feature will be limited."
                return
            }
            # Use old ItemsControl method
            $useOldMethod = $true
        } else {
            # Use new DataGrid method
            $useOldMethod = $false
            Write-DebugMessage "Found dgGroups DataGrid for AD groups"
        }
        ...
        if (-not $useOldMethod) {
            # For DataGrid, create items with IsSelected, Name, and Description properties
            foreach ($key in $adGroupsIni.Keys) {
                ...
                if (-not [string]::IsNullOrWhiteSpace($groupName)) {
                    ...
                    Write-DebugMessage "Added group for DataGrid: Name='$groupName', Description='$key'"
                }
            }
            
            # Set the ItemsSource for the DataGrid
            $dgGroups.ItemsSource = $adGroupItems
            Write-DebugMessage "AD groups loaded into DataGrid: $($adGroupItems.Count) groups."
...
function Get-SelectedADGroups {
    ...
    # Check if it's a DataGrid
    if ($Panel -is [System.Windows.Controls.DataGrid]) {
        Write-DebugMessage "Processing DataGrid for AD groups"
        
        if ($Panel.ItemsSource -eq $null -or $Panel.ItemsSource.Count -eq 0) {
            Write-DebugMessage "DataGrid contains no items to select from"
            return "NONE"
        }
        
        # For DataGrid, iterate through ItemsSource
        foreach ($item in $Panel.ItemsSource) {
            if ($item.IsSelected -eq $true) {
                $groupName = $item.Name
                Write-DebugMessage "Selected group from DataGrid: $groupName"
                [void]$selectedGroups.Add($groupName)
            }
        }
        
        Write-DebugMessage "Total selected groups from DataGrid: $($selectedGroups.Count)"
        ...
        # Handle both DataGrid and ItemsControl cases
    if ($Panel -is [System.Windows.Controls.DataGrid]) {
        # DataGrid logic handled above
        Write-DebugMessage "DataGrid processing completed"
        return "NONE" # DataGrid logic is handled above
    } elseif ($Panel -is [System.Windows.Controls.ItemsControl]) {
        # ItemsControl logic
...
function Invoke-Onboarding {
    ...
    # [16.1.14 - Extended Onboarding Features]
    Write-DebugMessage "Invoke-Onboarding: Processing extended features"
    
    # Create home directory if enabled
    if ($Config.OnboardingExtensions.CreateHomeDirectory -eq "1") {
        try {
            if (Get-Command New-HomeDirectory -ErrorAction SilentlyContinue) {
                $homePath = $Config.OnboardingExtensions.HomeDirectoryPath -replace "%username%", $SamAccountName
                $homeDir = New-HomeDirectory -SamAccountName $SamAccountName -HomePath (Split-Path $homePath) -SetInAD
                Write-DebugMessage "Home directory created: $homeDir"
            }
        }
        catch {
            Write-DebugMessage "Error creating home directory: $($_.Exception.Message)"
        }
    }
    
    # Send welcome email if enabled
    if ($Config.EmailSettings.SendWelcomeEmail -eq "1" -and -not [string]::IsNullOrWhiteSpace($email)) {
        try {
            if (Get-Command Send-WelcomeEmail -ErrorAction SilentlyContinue) {
                # Add password to userData for email
                $userData | Add-Member -MemberType NoteProperty -Name "Password" -Value $mainPW -Force
                $userData | Add-Member -MemberType NoteProperty -Name "Email" -Value $email -Force
                
                Send-WelcomeEmail -UserData $userData -Config $Config
                Write-DebugMessage "Welcome email sent to $email"
            }
        }
        catch {
            Write-DebugMessage "Error sending welcome email: $($_.Exception.Message)"
        }
    }
    
    # Generate detailed report if enabled
    if ($Config.OnboardingExtensions.GenerateDetailedReport -eq "1") {
        try {
            if (Get-Command New-UserReport -ErrorAction SilentlyContinue) {
                $detailedReportPath = Join-Path $reportPath "$SamAccountName-detailed.html"
                New-UserReport -Identity $SamAccountName -ReportType "Detailed" -OutputPath $detailedReportPath
                Write-DebugMessage "Detailed report generated: $detailedReportPath"
            }
        }
        catch {
            Write-DebugMessage "Error generating detailed report: $($_.Exception.Message)"
        }
    }
    
    # Trigger AD Sync if enabled
    if ($Config.ADSync.EnableADSync -eq "1" -and $Config.ADSync.AutoSyncNewUsers -eq "1") {
        try {
            Start-ADSyncProcess
            Write-DebugMessage "AD synchronization triggered"
        }
        catch {
            Write-DebugMessage "AD Sync failed: $($_.Exception.Message)"
        }
    }
...
#region [Region 18.5 | AD SYNC FUNCTIONS]
# Functions for AD synchronization
function Start-ADSyncProcess {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $false)]
        [switch]$Force
    ...
    try {
        Write-DebugMessage "Starting AD synchronization process"
        
        # Check if AD Sync is enabled
        if ($global:Config.ADSync.EnableADSync -ne "1" -and -not $Force) {
            Write-DebugMessage "AD Sync is disabled in configuration"
            return
        }
        
        # Get sync server
        $syncServer = $global:Config.ADSync.ADSyncServer
        if ([string]::IsNullOrWhiteSpace($syncServer)) {
            throw "AD Sync server not configured"
        }
        
        # Get sync command
        $syncCommand = $global:Config.ADSync.SyncCommand
        if ([string]::IsNullOrWhiteSpace($syncCommand)) {
            $syncCommand = "Start-ADSyncSyncCycle -PolicyType Delta"
        }
        
        Write-DebugMessage "Connecting to AD Sync server: $syncServer"
        
        # Execute sync command on remote server
        $scriptBlock = [ScriptBlock]::Create($syncCommand)
        $result = Invoke-Command -ComputerName $syncServer -ScriptBlock $scriptBlock -ErrorAction Stop
        
        Write-LogMessage -Message "AD synchronization initiated successfully on $syncServer" -LogLevel "INFO"
        return $true
    }
    catch {
        Write-LogMessage -Message "AD synchronization failed: $($_.Exception.Message)" -LogLevel "ERROR"
        throw
    }
}

function Test-ADSyncConnection {
    [CmdletBinding()]
    param()
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($syncServer)) {
            return @{
                Success = $false
                Message = "AD Sync server not configured"
            }
        }
        
        # Test connection to sync server
        $testResult = Test-Connection -ComputerName $syncServer -Count 1 -Quiet
        if (-not $testResult) {
            return @{
                Success = $false
                Message = "Cannot reach AD Sync server: $syncServer"
            }
        }
        
        # Test if we can run commands on the server
        try {
            $null = Invoke-Command -ComputerName $syncServer -ScriptBlock { Get-Date } -ErrorAction Stop
            return @{
                Success = $true
                Message = "AD Sync server connection successful"
            }
        }
        catch {
            return @{
                Success = $false
                Message = "Cannot execute commands on AD Sync server: $($_.Exception.Message)"
            }
        }
    }
    catch {
        return @{
            Success = $false
            Message = "Error testing AD Sync connection: $($_.Exception.Message)"
        }
    }
}
#endregion
...
#region [Region 21.3 | ONBOARDING PANEL]
# Implements the main onboarding panel functionality
Write-DebugMessage "Onboarding panel loaded."
$onboardingPanel = $window.FindName("onboardingPanel")
# [21.3.1 - Validates the panel exists in the XAML interface]
if (-not $onboardingPanel) {
    Write-DebugMessage "ERROR: Onboarding-Panel missing!"
    Write-Error "The Onboarding-Panel (x:Name='onboardingPanel') was not found in the XAML."
    exit
}
# Use the panel as the reference for finding controls
$onboardingTab = $onboardingPanel
#endregion
...
$btnStartOnboarding = $btnCreateUser  # Alias for compatibility

# [21.4.1 - Sets up the primary function to execute when user initiates onboarding]
if (-not $btnCreateUser) {
    Write-DebugMessage "ERROR: btnCreateUser was NOT found!"
    # Try old button name as fallback
    $btnStartOnboarding = $onboardingTab.FindName("btnOnboard")
    if (-not $btnStartOnboarding) {
        Write-DebugMessage "ERROR: Neither btnCreateUser nor btnOnboard found!"
        exit
    }
} else {
    $btnStartOnboarding = $btnCreateUser
}

Write-DebugMessage "Registering event handler for Create User button"
$btnStartOnboarding.Add_Click({
    Write-DebugMessage "Onboarding button clicked!"

    try {
        # Find the AD groups control for selection
        $dgGroups = $window.FindName("dgGroups")
        $ADGroupsSelected = if ($dgGroups) { 
            # Get selected groups from DataGrid
            Get-SelectedADGroups -Panel $dgGroups 
        } else {
            # Try old ItemsControl as fallback
        ...
        try {
            ...
            if ($openFileDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
                ...
                try {
                    ...
                    $dgGroups = $window.FindName("dgGroups")
                    if ($dgGroups -and $dgGroups.ItemsSource) {
                        foreach ($item in $dgGroups.ItemsSource) {
                            $item.IsSelected = $false
                        }
                    } elseif ($icADGroups -and $global:ADGroupItems) {
                    ...
                    if ($selectedRow.PSObject.Properties.Name -contains "ADGroup") {
                        ...
                        if ($dgGroups -and $dgGroups.ItemsSource) {
                            foreach ($item in $dgGroups.ItemsSource) {
                                if ($item.Name -eq $adGroupName -or $item.Description -eq $adGroupName) {
                                    $item.IsSelected = $true
                                    Write-DebugMessage "Selected AD Group in DataGrid: $($item.Name)"
                                    break
                                }
                            }
                        }
                        # Fallback to ItemsControl
                        elseif ($icADGroups -and $global:ADGroupItems) {
                            foreach ($item in $global:ADGroupItems) {
...
#region [Region 21.8 | AD UPDATE PANEL]
# Implements the AD Update panel functionality
Write-DebugMessage "AD Update panel loaded."

# Find the AD Update panel
$adUpdatePanel = $window.FindName("userupdatePanel")
if ($adUpdatePanel) {
    # Use the panel as the reference for finding controls
    $adUpdateTab = $adUpdatePanel
    ...
    if ($btnADUserUpdate) {
        ...
        # Trigger AD Sync if enabled
        if ($global:Config.ADSync.EnableADSync -eq "1" -and $global:Config.ADSync.AutoSyncNewUsers -eq "1") {
            try {
                Start-ADSyncProcess
            }
            catch {
...
# Navigation Panel Button Handlers
$btnDashboard = $window.FindName("btnDashboard")
$btnOnboarding = $window.FindName("btnOnboarding")
$btnUserUpdate = $window.FindName("btnUserUpdate")
$btnCSVImport = $window.FindName("btnCSVImport")
$btnReports = $window.FindName("btnReports")
...
function Switch-Panel {
    ...
    # Hide all panels
    @("dashboardPanel", "onboardingPanel", "userupdatePanel", "csvimportPanel", "reportsPanel", "toolsPanel", "settingsPanel") | ForEach-Object {
        $panel = $window.FindName($_)
        if ($panel) { $panel.Visibility = "Collapsed" }
    }
    
    # Show selected panel
    $selectedPanel = $window.FindName("${PanelName}Panel")
    if ($selectedPanel) { 
        $selectedPanel.Visibility = "Visible" 
        Write-DebugMessage "Switched to panel: $PanelName"
    }
}
...
if ($btnDashboard) {
    $btnDashboard.Add_Click({
        Switch-Panel -PanelName "dashboard"
    })
}

if ($btnOnboarding) {
    $btnOnboarding.Add_Click({
        Switch-Panel -PanelName "onboarding"
    })
}

if ($btnUserUpdate) {
    $btnUserUpdate.Add_Click({
        Switch-Panel -PanelName "userupdate"
    })
}

if ($btnCSVImport) {
    $btnCSVImport.Add_Click({
        Switch-Panel -PanelName "csvimport"
    })
}

if ($btnReports) {
    $btnReports.Add_Click({
        Switch-Panel -PanelName "reports"
    })
}

if ($btnTools) {
    $btnTools.Add_Click({
        Switch-Panel -PanelName "tools"
    })
}

if ($btnSettings) {
    $btnSettings.Add_Click({
        Switch-Panel -PanelName "settings"
    })
}

# Report Panel Event Handlers
$btnGenerateReport = $window.FindName("btnGenerateReport")
if ($btnGenerateReport) {
    $btnGenerateReport.Add_Click({
        try {
            Write-DebugMessage "Generate Report button clicked"
            
            # Get selected report type
            $reportType = ""
            if ($window.FindName("rbOnboardingReport").IsChecked) { $reportType = "Onboarding" }
            elseif ($window.FindName("rbUserListReport").IsChecked) { $reportType = "UserList" }
            elseif ($window.FindName("rbLicenseReport").IsChecked) { $reportType = "License" }
            elseif ($window.FindName("rbGroupMembershipReport").IsChecked) { $reportType = "GroupMembership" }
            elseif ($window.FindName("rbInactiveUsersReport").IsChecked) { $reportType = "InactiveUsers" }
            elseif ($window.FindName("rbAuditReport").IsChecked) { $reportType = "Audit" }
            elseif ($window.FindName("rbErrorReport").IsChecked) { $reportType = "Error" }
            elseif ($window.FindName("rbPerformanceReport").IsChecked) { $reportType = "Performance" }
            
            # Get report period
            $period = $window.FindName("cmbReportPeriod").SelectedItem.Content
            
            # Get report format
            $format = $window.FindName("cmbReportFormat").SelectedItem.Content
            
            Write-Log "Generating $reportType report for period: $period in format: $format" "INFO"
            
            # Generate report based on type
            switch ($reportType) {
                "Onboarding" {
                    # Use the ReportingModule to generate onboarding summary
                    if (Get-Command "New-OnboardingSummaryReport" -ErrorAction SilentlyContinue) {
                        $endDate = Get-Date
                        $startDate = switch ($period) {
                            "Heute" { $endDate.Date }
                            "Diese Woche" { $endDate.AddDays(-7) }
                            "Dieser Monat" { $endDate.AddMonths(-1) }
                            "Letzter Monat" { 
                                $firstDayLastMonth = (Get-Date).AddMonths(-1).Date.AddDays(-((Get-Date).AddMonths(-1).Day - 1))
                                $lastDayLastMonth = $firstDayLastMonth.AddMonths(1).AddDays(-1)
                                $startDate = $firstDayLastMonth
                                $endDate = $lastDayLastMonth
                                $startDate
                            }
                            "Dieses Jahr" { Get-Date -Day 1 -Month 1 }
                            default { $endDate.AddDays(-7) }
                        }
                        
                        $report = New-OnboardingSummaryReport -StartDate $startDate -EndDate $endDate -OutputFormat $format
                        
                        if ($report.Success) {
                            # Enable export button
                            $window.FindName("btnExportReport").IsEnabled = $true
                            $window.FindName("btnEmailReport").IsEnabled = $true
                            
                            # Show preview if HTML
                            if ($format -eq "HTML" -and (Test-Path $report.Path)) {
                                $window.FindName("wbReportPreview").Navigate($report.Path)
                            }
                            
                            [System.Windows.MessageBox]::Show("Report erfolgreich generiert!`n`nPfad: $($report.Path)", "Erfolg", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
                        }
                    } else {
                        [System.Windows.MessageBox]::Show("ReportingModule nicht verfügbar.", "Fehler", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
                    }
                }
                
                "License" {
                    # Generate license usage report
                    if (Get-Command "Get-LicenseUsageReport" -ErrorAction SilentlyContinue) {
                        $report = Get-LicenseUsageReport -IncludeServicePlans
                        
                        if ($report) {
                            # Convert to desired format and save
                            $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
                            $reportPath = Join-Path $scriptPath "Reports\LicenseReport_$timestamp.$($format.ToLower())"
                            
                            switch ($format) {
                                "CSV" { $report | Export-Csv -Path $reportPath -NoTypeInformation }
                                "Excel" { 
                                    if (Get-Module -ListAvailable -Name ImportExcel) {
                                        $report | Export-Excel -Path $reportPath -AutoSize -AutoFilter 
                                    } else {
                                        $report | Export-Csv -Path $reportPath -NoTypeInformation
                                        [System.Windows.MessageBox]::Show("Excel-Export nicht verfügbar. Als CSV gespeichert.", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
                                    }
                                }
                                "HTML" {
                                    # Generate HTML report
                                    $html = ConvertTo-Html -InputObject $report -Title "License Usage Report" -PreContent "<h1>License Usage Report</h1><p>Generated: $(Get-Date)</p>"
                                    Set-Content -Path $reportPath -Value $html
                                    $window.FindName("wbReportPreview").Navigate($reportPath)
                                }
                            }
                        ...
                        [System.Windows.MessageBox]::Show("LicenseModule nicht verfügbar.", "Fehler", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
                    }
                }
                
                # Add more report types as needed
                default {
                    [System.Windows.MessageBox]::Show("Dieser Report-Typ ist noch nicht implementiert.", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
                }
            }
        }
        catch {
            Write-Log "Error generating report: $_" "ERROR"
            [System.Windows.MessageBox]::Show("Fehler beim Generieren des Reports: $_", "Fehler", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
        }
    })
}

# Tools Panel Button Handlers
$btnBulkPasswordReset = $window.FindName("btnBulkPasswordReset")
if ($btnBulkPasswordReset) {
    $btnBulkPasswordReset.Add_Click({
        Write-DebugMessage "Bulk Password Reset clicked"
        # TODO: Implement bulk password reset functionality
        [System.Windows.MessageBox]::Show("Bulk Password Reset - In Entwicklung", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
    })
}

$btnBulkLicenseAssignment = $window.FindName("btnBulkLicenseAssignment")
if ($btnBulkLicenseAssignment) {
    $btnBulkLicenseAssignment.Add_Click({
        Write-DebugMessage "Bulk License Assignment clicked"
        # Check if LicenseModule is available
        if (Get-Command "Add-BulkUserLicenses" -ErrorAction SilentlyContinue) {
            # TODO: Create UI for bulk license assignment
            [System.Windows.MessageBox]::Show("Bulk License Assignment - UI in Entwicklung", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
        } else {
            [System.Windows.MessageBox]::Show("LicenseModule nicht verfügbar.", "Fehler", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Warning)
        }
    })
...
$btnADHealthCheck = $window.FindName("btnADHealthCheck")
if ($btnADHealthCheck) {
    $btnADHealthCheck.Add_Click({
        Write-DebugMessage "AD Health Check clicked"
        # TODO: Implement AD health check
        [System.Windows.MessageBox]::Show("AD Health Check - In Entwicklung", "Information", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
    })
}
...
if ($btnSettings1) {
    ...
    $btnSettings1.Add_Click({
        try {
            Write-DebugMessage "Settings button clicked - switching to Settings panel"
            
            # Try to use new Settings Panel first
            if (Get-Command "Show-SettingsPanel" -ErrorAction SilentlyContinue) {
                Show-SettingsPanel
            }
            # Check if we have UIManager navigation
            elseif (Get-Command "Switch-ToPanel" -ErrorAction SilentlyContinue) {
                Switch-ToPanel -PanelName "Settings"
            }
            # Fallback to external INI editor
            else {
                Write-DebugMessage "New Settings Panel not available, launching external INI editor"
                Open-INIEditor
                $window.Close()
            }
        } catch {
            Write-DebugMessage "Error opening Settings: $($_.Exception.Message)"
            # Fallback to external INI editor on error
            try {
                Open-INIEditor
                $window.Close()
            } catch {
                Write-DebugMessage "Error opening INI editor fallback: $($_.Exception.Message)"
            }
        }
    })
}
...
# Initialize custom modules functionality
try {
    Write-DebugMessage "Initializing custom modules functionality"
    
    # Initialize SettingsManager if available
    if (Get-Command "Initialize-SettingsManager" -ErrorAction SilentlyContinue) {
        Initialize-SettingsManager -ConfigPath $INIPath -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
        Write-DebugMessage "SettingsManager initialized"
    }
    
    # Initialize UIManager if available
    if (Get-Command "Initialize-UIManager" -ErrorAction SilentlyContinue) {
        Initialize-UIManager -Window $window
        Write-DebugMessage "UIManager initialized"
    }
    
    # Initialize ADManager if available
    if (Get-Command "Initialize-ADManager" -ErrorAction SilentlyContinue) {
        if ($global:ADConnectionAvailable) {
            Initialize-ADManager -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level }
            Write-DebugMessage "ADManager initialized with AD connectivity"
        } else {
            Write-DebugMessage "ADManager available but AD connectivity limited - initializing in offline mode"
            Initialize-ADManager -LogFunction { param($msg, $level) Write-Log -Message $msg -LogLevel $level } -OfflineMode
        }
    }
    
    # Initialize Settings Panel if available
    if (Get-Command "Initialize-SettingsPanel" -ErrorAction SilentlyContinue) {
        Initialize-SettingsPanel -Window $window
        Write-DebugMessage "Settings Panel initialized"
    }
    
    Write-DebugMessage "Custom modules functionality initialized"
} catch {
    Write-DebugMessage ("Error initializing custom modules functionality: " + $_.Exception.Message)
    Write-Log -Message "Error initializing custom modules functionality: $($_.Exception.Message)" -LogLevel "WARN"
}

# Display AD connectivity status to user
try {
    $adStatus = Get-ADConnectionStatus
    if ($adStatus) {
        Write-DebugMessage "✓ Active Directory connectivity: Available"
        Write-Log -Message "Active Directory connectivity established successfully" -LogLevel "INFO"
    } else {
        Write-DebugMessage "⚠ Active Directory connectivity: Limited"
        Write-Log -Message "Active Directory connectivity limited - some features may not be available" -LogLevel "WARN"
        
        # Show user-friendly message about limited functionality
        if ($global:Config.Logging.DebugMode -eq "1") {
            Write-Host "⚠ WARNING: Active Directory connectivity is limited. Some features may not work properly." -ForegroundColor Yellow
        }
    }
} catch {
    Write-DebugMessage "Error checking AD connectivity status: $($_.Exception.Message)"
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCnrbnABbm3XA+6
# ba6FH2SAUmg1GQNO6kdyzMEX2pIXnKCCFk4wggMQMIIB+KADAgECAhB3jzsyX9Cg
...
# BgkqhkiG9w0BCQQxIgQgDj3Uzn5aiG/0CDEqVVa+KDrg8YSruMTJyrkdCLwVyGMw
# DQYJKoZIhvcNAQEBBQAEggEAfCUtip+6OfrpG/A0sCR+OILdBpmpCVmzrWU6xrDB
# 8FfRDVT1egWDt6sKjaSgrKPSl71N0XIeJWaQ3byUK4kOlKdFHVcDcjdZn8efNVvT
# zcmuUk0T2gxvT/1QNdaPPYpKJku0qAysw2ZYG3p5Ps9BfekcpOsoqDZkSyx1rabY
# qpZKxV5No8XvgY4988bxlutQgH4Zj6xpOQC6ILERMBL/I4IZ86eDcvpD8ChfYfiN
# uwCB8UIkDBf1XbvhEdEKtQ+NfTsdd4ciReUvMfjUgiLVwJC0ErkqsJtKDLsBI6Wj
# lUk5L8Zocp2Xcdbz3OC7dIwK/Va8CssbT+poVKYJjGtAmKGCAyYwggMiBgkqhkiG
...
# SIb3DQEJBTEPFw0yNTA3MTIxMDIyMDdaMC8GCSqGSIb3DQEJBDEiBCDuC6Ck/eLm
# vDC3oqpxEcUk7cxk93oIgzq/T8eSHPv4VzANBgkqhkiG9w0BAQEFAASCAgAXRt67
# m7+tecpEb5LaNaPe0HJ37di3l1XIO9HU3laCFmO1V9fPwPJVqbnO/FLeRgh2tCHr
# aIoeUB3W/+5Wtfm04EKTtvK1TaC2Vu+XsCKMXvlDcddNjettl6TtmsYA2uTRAwSx
# Ufimlo0lq3wJU/Jni/GVVvin7ivJfJinK/gZNGYLxdo48xqh4OYSzWY3in5Tvsgg
# MzTOngOnJcoNww9BvQSbAA3OIcfUIr6UOM2ikAFSQEKx3nZjLZPyFQgVQwDut0Ks
# yu/YeERskNmPesl1RX2NwgVzc0F+xvP/EcvQsJs4H3vnDnrtLR3H88FgCtj2SzUB
# yqFWy6ZAVWdDbevaTGnKgL3RkuceVNux2ZZIZCPpzt1SiJoxOl/Ns1R+1xDR+2Fy
# UxxoW+JjfmSqFfakNnZJgQ0Uavqv+AtHPEOL+kNYf0yBmKn9tYYO5+pqD7oTnh9s
# 0/hrf7HBM0oqcmgmQwzV3sOq4iZxv7hUdX8EyIudIJKWbKJztjY670KGaGxAtexq
# I5W8vvPQvaCrcbaq7yFH4/ITfVmuOzj6oVbsbnjylzvlAqu4SaYfOJC2i4FucDoL
# bRTiGw1dlF3SG0+KrRAP4+0eGpueGjh+YldwomZw/eOPKvaexBNN29xG8roViZyy
# o2t6eWfQAlJVpXi2xYe53Cq//sT8BWjbdCZybQ==
