...
if ($tabINIEditor) {
    ...
    if ($btnINIEditorSaveChanges) {
        $btnINIEditorSaveChanges.Add_Click({
            try {
                # Store current selection before saving
                $currentSelectedSection = $null
                if ($listViewINIEditor.SelectedItem -ne $null) {
                    $currentSelectedSection = $listViewINIEditor.SelectedItem.SectionName
                    Write-DebugMessage "Current selected section before saving: $currentSelectedSection"
                }
                ...
                if ($result) {
                    ...
                    # No need to reload the entire INI file from disk since we already have the updated data in $global:Config
                    # We just need to refresh the UI
                    
                    # First refresh the section list
                    Import-INIEditorData
                    
                    # Then restore the selection if we had one
                    if ($currentSelectedSection -ne $null) {
                        # Find and select the section item
                        foreach ($item in $listViewINIEditor.Items) {
                            if ($item.SectionName -eq $currentSelectedSection) {
                                $listViewINIEditor.SelectedItem = $item
                                Write-DebugMessage "Restored selection to: $currentSelectedSection"
                                break
                            }
                        }
                        
                        # If the selection was restored, refresh the section settings
                        if ($listViewINIEditor.SelectedItem -ne $null) {
                            Import-SectionSettings -SectionName $currentSelectedSection
                        }
                    }
                }
                else {
