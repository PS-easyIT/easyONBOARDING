<#
.SYNOPSIS
    Module to load and process XAML for WPF GUI
.DESCRIPTION
    This module provides functions to load and process XAML files for WPF GUI applications,
...
# Import required assemblies for WPF if not already loaded
$requiredAssemblies = @(
...
foreach ($assembly in $requiredAssemblies) {
    try {
        if (-not ([System.AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { $_.FullName -like "*$assembly*" })) {
            Add-Type -AssemblyName $assembly -ErrorAction Stop
            if (Get-Command -Name "Write-DebugMessage" -ErrorAction SilentlyContinue) {
                Write-DebugMessage "Assembly loaded: $assembly" -LogLevel "INFO"
            }
            else {
                Write-Host "Assembly loaded: $assembly" -ForegroundColor Green
            }
        }
    }
    catch {
        if (Get-Command -Name "Write-DebugMessage" -ErrorAction SilentlyContinue) {
            Write-DebugMessage "Failed to load assembly: $assembly - $($_.Exception.Message)" -LogLevel "ERROR"
        }
        else {
            Write-Host "Failed to load assembly: $assembly - $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
...
# Main function to import XAML window
function Import-XamlWindow {
    ...
    if (-not (Test-Path $XamlPath)) {
        $errorMsg = "XAML file not found: $XamlPath"
        ...
        if (-not $NoErrorDialog) {
            [System.Windows.MessageBox]::Show($errorMsg, "File Not Found", "OK", "Error")
        }
    ...
    try {
        ...
        # Create XML reader with proper namespace handling
        [xml]$xaml = $xamlContent
        
        # Remove x:Class attribute to prevent loading errors
        if ($xaml.DocumentElement.HasAttribute("Class") -or $xaml.DocumentElement.HasAttribute("x:Class")) {
        ...
        if (Get-Command -Name "Write-DebugMessage" -ErrorAction SilentlyContinue) {
            Write-DebugMessage "XAML loaded successfully from: $XamlPath" -LogLevel "INFO"
        }
        else {
            Write-Host "XAML loaded successfully from: $XamlPath" -ForegroundColor Green
        }
        
        # Process XAML names for easy access - store as variables if requested
        ...
        $errorMsg = "Error loading XAML from $XamlPath : $($_.Exception.Message)"
        
        if (Get-Command -Name "Write-DebugMessage" -ErrorAction SilentlyContinue) {
            Write-DebugMessage $errorMsg -LogLevel "ERROR"
            Write-DebugMessage "Exception details: $($_.Exception.ToString())" -LogLevel "DEBUG"
        }
        else {
            Write-Host $errorMsg -ForegroundColor Red
            Write-Host "Exception details: $($_.Exception.ToString())" -ForegroundColor DarkRed
        }
        ...
        if (-not $NoErrorDialog) {
            ...
            try {
                ...
                $errorWindow.Width = 600
                $errorWindow.Height = 400
                $errorWindow.Content = $errorMsg
...
Export-ModuleMember -Function Import-XamlFromFile, Get-XamlNamedElements, Load-XamlInterface, Get-XamlFilePathCandidates, Import-XAMLGUI, Initialize-WPFAssemblies, Import-XamlWindow