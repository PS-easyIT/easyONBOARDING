...
function Show-OnboardingForm {
    ...
    # Neuer Bereich: Konto läuft ab (Ablaufdatum)
    AddLabel $panelLeft "Konto läuft ab:" 10 $yLeft -Bold | Out-Null
    $txtAblaufdatum = AddTextBox $panelLeft "" 150 $yLeft; $yLeft += 30
    ...
    # ==================== Angepasste $btnOnboard Click Routine ====================
    $btnOnboard.Add_Click({
        ...
        if (($INIConfig[$selectedCompany.Section].Keys -contains ("NameFirma" + $suffix)) -and `
            -not [string]::IsNullOrWhiteSpace($INIConfig[$selectedCompany.Section]["NameFirma" + $suffix])) {
            $prefix = $INIConfig[$selectedCompany.Section]["NameFirma" + $suffix].Trim()
        } else {
            $prefix = $selectedCompany.Display
        }
        # Angepasste DisplayName-Ermittlung:
        ...
        elseif ($cmbDisplayNameTemplate.SelectedItem -and $cmbDisplayNameTemplate.SelectedItem -match '{first}') {
            ...
            $DisplayName = $template -replace '{first}', $txtVorname.Text -replace '{last}', $txtNachname.Text
        }
        else {
...
if (-not $finalReportFooter) {
    $finalReportFooter = $reportFooter
}

# Branding-Report-Einstellungen
$firmaLogoPath = $reportBranding["ReportLogo"]
$headerText   = $reportBranding["ReportHeader"]
$footerText   = $reportBranding["ReportFooter"]
...
if ($UPNManual) { 
    ...
    if ( ($Config.Keys -contains $companySection) -and 
         ($companyData.Keys -contains ("ActiveDirectoryDomain" + $suffix)) -and 
...
# Korrigiere: statt "LocalProfilePath" wird hier "HomeDirectory" genutzt
if ($Config.ADUserDefaults.Contains("HomeDirectory")) {
    $userParams["HomeDirectory"] = $Config.ADUserDefaults["HomeDirectory"]
}
...
try {
    ...
    if ($createHTML) {
        ...
        if ($firmaLogoPath -and (Test-Path $firmaLogoPath)) {
            $logoTag = "<div class='logo'><img src='file:///$firmaLogoPath' style='max-width:150px; margin:10px;'/></div>"
        }
        ...
        $userDetails = [ordered]@{
            ...
            "Ablaufdatum"     = $userSelection.Ablaufdatum
        ...
        # Webseiten aus der INI
        $websitesHTML = ""
        if ($Config.Contains("Websites")) {
            $websitesHTML = "<table class='websites-table'>
                <colgroup>
                <col style='width:20%;'>
                <col style='width:50%;'>
                <col style='width:30%;'>
            ...
            foreach ($wKey in $Config.Websites.Keys) {
                if ($wKey -match '^EmployeeLink\d+$') {
                    $line = $Config.Websites[$wKey]
                    $parts = $line -split ';'
                    if ($parts.Count -eq 3) {
                        $title = $parts[0].Trim()
                        $url   = $parts[1].Trim()
                        $desc  = $parts[2].Trim()
                        $websitesHTML += "<tr>"
                        $websitesHTML += "<td>$title</td>"
                        $websitesHTML += "<td><a href='$url' target='_blank'>$url</a></td>"
                        $websitesHTML += "<td>$desc</td>"
                        $websitesHTML += "</tr>"
                    }
                }
            }
        ...
        # Setze Standardwerte, falls Felder NULL sind
        if (-not $userSelection.UPNFormat) { $userSelection.UPNFormat = "" }
        if (-not $userSelection.FixPassword) { $userSelection.FixPassword = "" }
        if (-not $Description) { $Description = "" }

        # Ternäre Operatoren ersetzen
        if ($userSelection.Extern) {
            $externStr = "Ja"
        } else {
            $externStr = "Nein"
        }
        if ($userSelection.PasswordMode -eq 1) {
            $passwordModeStr = "GENERIERT"
        } else {
            $passwordModeStr = "FEST"
        }

        # Ersetze Platzhalter im Template
        $htmlContent = $htmlTemplate `
        -replace "{{ReportTitle}}", ([string]$reportTitle) `
        -replace "{{LogoTag}}", ([string]$logoTag) `
        -replace "{{UserDetailsHTML}}", ([string]$userDetailsHTML) `
        -replace "{{WebsitesHTML}}", ([string]$websitesHTML) `
        -replace "{{ReportFooter}}", ([string]$finalReportFooter) `
        -replace "{{Vorname}}", ([string]$Vorname) `
        -replace "{{Nachname}}", ([string]$Nachname) `
        -replace "{{DisplayName}}", ([string]$DisplayName) `
        -replace "{{Extern}}", ([string]$externStr) `
        -replace "{{Description}}", ([string]$Description) `
        -replace "{{Buero}}", ([string]$OfficeRoom) `
        -replace "{{Rufnummer}}", ([string]$PhoneNumber) `
        -replace "{{Mobil}}", ([string]$MobileNumber) `
        -replace "{{Position}}", ([string]$Position) `
        -replace "{{Abteilung}}", ([string]$DepartmentField) `
        -replace "{{Ablaufdatum}}", ([string]$userSelection.Ablaufdatum) `
        -replace "{{Company}}", ([string]$companyDisplay) `
        -replace "{{LoginName}}", ([string]$SamAccountName) `  
        -replace "{{Passwort}}", ([string]$UserPW) `        
        -replace "{{MS365License}}", ([string]$MS365License) `
        -replace "{{UPN}}", ([string]$UPN) `
        -replace "{{UPNFormat}}", ([string]$userSelection.UPNFormat) `
        -replace "{{EmailAddress}}", ([string]$EmailAddress) `
        -replace "{{MailSuffix}}", ([string]$mailSuffix) `
        -replace "{{PasswordNeverExpires}}", ([string]$userSelection.PasswordNeverExpires) `
        -replace "{{MustChangePassword}}", ([string]$userSelection.MustChangePassword) `
        -replace "{{AccountDisabled}}", ([string]$userSelection.AccountDisabled) `
        -replace "{{CannotChangePassword}}", ([string]$userSelection.CannotChangePassword) `
        -replace "{{SmartcardLogonRequired}}", ([string]$userSelection.SmartcardLogonRequired) `
        -replace "{{PasswordMode}}", ([string]$passwordModeStr) `
        -replace "{{FixPassword}}", ([string]$userSelection.FixPassword) `
        -replace "{{PasswordLength}}", ([string]$userSelection.PasswordLaenge) `
        -replace "{{IncludeSpecialChars}}", ([string]$userSelection.IncludeSpecialChars) `
        -replace "{{AvoidAmbiguousChars}}", ([string]$userSelection.AvoidAmbiguousChars) `
        -replace "{{ADGroups}}", ([string]($userSelection.ADGroupsSelected -join ", ")) `
        -replace "{{Admin}}", ([string]$env:USERNAME) `
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC